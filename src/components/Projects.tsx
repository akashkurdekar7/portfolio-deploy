...
const Projects = () => {
  ...
  // HEADING SCRAMBLE — "Projects" and its "0N" badge scramble in from random
  ...
  useScrambleReveal([
    {
      ...
      start: "top 80%",
      targets: [
        { ref: headingRef, text: "Projects" },
        { ref: badgeRef, text: `0${projects.length}` },
      ],
    },
  ]);

  // DESCRIPTION WORD REVEAL — same scroll-scrubbed word fade used in Work's
  // descriptions: words start at opacity 0 and ramp to 1 as this paragraph
  // scrolls through the viewport.
  useLayoutEffect(() => {
    if (window.matchMedia("(prefers-reduced-motion: reduce)").matches) return;

    const ctx = gsap.context(() => {
      const words = document.querySelectorAll(".projects-desc-word");
    ...
    <section id="projects" ref={sectionRef} className="relative mx-5 min-h-screen overflow-hidden py-16 lg:py-24 md:mx-20">
      ...
      <div className="flex flex-col items-center gap-3">
        <h2 className="size56 font-bricolage-semibold leading-none capitalize ">
          <span ref={headingRef}>Projects</span>
          <sup ref={badgeRef} className="size18 align-super  font-bricolage ml-1 text-white bg-blue rounded-full   px-2 ">
            0{projects.length}
          </sup>
        </h2>

        <p className="projects-desc max-w-lg text-center font-bricolage size14 leading-4">
          {renderEmphasisText(PROJECTS_DESCRIPTION, "projects-desc-word")}
        </p>
      </div>
