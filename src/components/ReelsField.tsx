...
const ReelsField = () => {
  ...
  useEffect(() => {
    ...
    // Loaded lazily so three.js (unused by anything on first paint) doesn't
    // inflate the main bundle that has to be parsed/evaluated before render.
    import("three").then((ThreeLib) => {
      ...
      const material = new ThreeLib.ShaderMaterial({
        ...
        uniforms: {
          uTime: { value: 0 },
          uMouse: { value: new ThreeLib.Vector2(0, 0) },
          uMouseActive: { value: 0 },
          uColorBase: { value: new ThreeLib.Color("#111111") },
          uColorAccent: { value: new ThreeLib.Color("#ff5a1f") },
        },
        vertexShader: `
          uniform float uTime;
          uniform vec2 uMouse;
      ...
      const buildGrid = (width: number, height: number) => {
        ...
        geometry = new ThreeLib.BufferGeometry();
        geometry.setAttribute("position", new ThreeLib.BufferAttribute(positions, 3));
        geometry.setAttribute("aRandom", new ThreeLib.BufferAttribute(randoms, 1));

        points = new ThreeLib.Points(geometry, material);
        scene.add(points);
      };
      ...
      const mouseTarget = new ThreeLib.Vector2(0, 0);
